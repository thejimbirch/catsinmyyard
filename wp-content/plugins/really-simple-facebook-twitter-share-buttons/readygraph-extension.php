<?php
  ...
  // Extension Configuration
	...
	$rsftsb_main_plugin_title = 'Really Simple Share';

	add_action( 'wp_ajax_nopriv_rsftsb-myajax-submit', 'rsftsb_myajax_submit' );
...
function on_plugin_activated_readygraph_rsftsb_redirect(){
	...
	$setting_url="admin.php?page=$rsftsb_menu_slug";    
...
function add_rsftsb_readygraph_plugin_warning() {
  if (get_option('readygraph_access_token', '') != '') return;
  global $hook_suffix, $current_user, $rsftsb_menu_slug;
  ...
  if ( $hook_suffix == 'plugins.php' && get_option('readygraph_connect_notice') == 'true' ) {              
    echo '<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">  
      ...
      <form name="readygraph_activate" action="'.admin_url( 'admin.php?page=' . $rsftsb_menu_slug).'" method="POST"> 
...
function rsftsb_post_updated_send_email( $post_id ) {
	...
	if(get_option('rsftsb_wordpress_sync_users')) return;
	else {
		add_action('plugins_loaded', 'rsftsb_wordpress_sync_users');
	}
function rsftsb_wordpress_sync_users() {
	if(get_option('rsftsb_wordpress_sync_users') && get_option('rsftsb_wordpress_sync_users') == "true") return;
	else {
		if(get_option('readygraph_application_id') && strlen(get_option('readygraph_application_id')) > 0){
			global $wpdb;
			$app_id = get_option('readygraph_application_id');
			...
			update_option('rsftsb_wordpress_sync_users',"true");
			remove_action('plugins_loaded', 'rsftsb_wordpress_sync_users');
		}
    }
}
function rsftsb_rg_connect(){
	...
	$randon_string = rsftsb_get_random_string();
...
function rsftsb_get_random_string()
{
